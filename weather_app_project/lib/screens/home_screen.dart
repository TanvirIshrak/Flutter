...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _unsubscribe() async {
    ...
    // Track the loader dialog via its own Navigator so we can close it
    // without affecting the underlying HomeScreen route.
    ...
    Future<void> closeLoader() async {
      ...
      if (ctx == null) return;
      // `useRootNavigator: false` matches the showDialog call above; pop the
      // dialog off the *home* navigator so the loader can never be orphaned
      // when we later navigate away with pushNamedAndRemoveUntil.
      if (!Navigator.of(ctx, rootNavigator: false).mounted) return;
      Navigator.of(ctx, rootNavigator: false).pop();
    }

    // Use the root navigator for the final route swap so the home screen (and
    // any dialog still pending on its navigator) is removed cleanly. This
    // prevents the black screen caused by an orphaned loader dialog.
    final rootNavContext = context;
    ...
    try {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(rootNavContext).showSnackBar(
          const SnackBar(
        ...
        // that triggered `_history.isNotEmpty` assertion failures. Going
        // through the root navigator guarantees the orphaned loader dialog
        // (if any) is removed along with HomeScreen.
        Navigator.of(
          rootNavContext,
          rootNavigator: true,
      ...
      appBar: AppBar(
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: _unsubscribe, // call unsubscribe function
